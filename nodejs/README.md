...
# OpenAPI Generated JavaScript/Express Server — Petstore (Node.js)

## Database / Persistence

This server persists data in a shared PostgreSQL instance (the `nodejs-express` database).

### Prerequisites

- Docker (with Compose v2) — to run the database
- `psql` client — to apply the schema
...
The server reads the following environment variables (with defaults matching `database/.env`):
