...
- Server-assigned IDs use `COALESCE(MAX(id), 0) + 1`
...
This mirrors the situation with `cargo-mutants` for the Rust server, which also has limited coverage documented in `rust/AGENTS.md`.
