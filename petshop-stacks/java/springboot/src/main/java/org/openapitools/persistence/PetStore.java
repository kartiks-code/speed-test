...
public class PetStore {
    ...
    private long nextId(String table) {
        String seq = switch (table) {
            case "pet" -> "pet_id_seq";
            case "\"order\"" -> "order_id_seq";
            case "\"user\"" -> "user_id_seq";
            default -> throw new IllegalArgumentException("no sequence for table: " + table);
        };
        Long id = jdbc.queryForObject("SELECT nextval('" + seq + "')", Long.class);
