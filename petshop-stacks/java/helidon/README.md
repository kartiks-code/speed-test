...
Request handling runs on **virtual threads by default** (Helidon MP 4's WebServer is Loom-based), so no extra configuration is needed. Because of this, the HikariCP connection pool is sized large (default 200, via `HIKARI_MAXIMUM_POOL_SIZE`) rather than capped like a platform-thread pool, so it does not bottleneck virtual-thread concurrency. The optimized Docker image uses **G1GC** (previously ZGC); under the perf harness's `512m` memory limit the heap is small (~384MB), where G1 is more memory-efficient than ZGC.
...
cd ../../../petshop-stacks/java/helidon
```
...
| `HIKARI_MAXIMUM_POOL_SIZE` | `200` | HikariCP max pool size; kept large because requests run on virtual threads |
