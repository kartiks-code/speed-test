...
func TestLogoutUserAlwaysOK(t *testing.T) {
	...
	app.Get("/api/v3/user/logout", api.LogoutUser)
	resp, err := app.Test(mustNewRequest(http.MethodGet, "/api/v3/user/logout"), -1)
	if err != nil {
		t.Fatalf("app.Test: %v", err)
	}
	resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("LogoutUser status = %d, want %d", resp.StatusCode, http.StatusOK)
	}
}
