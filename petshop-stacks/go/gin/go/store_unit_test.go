...
func TestPostgresDSNFromEnv(t *testing.T) {
	// Clear any inherited connection env so the assembled-DSN path is deterministic.
	...
	t.Run("assembles from parts with escaping", func(t *testing.T) {
		...
		t.Setenv("POSTGRES_DB", "go-gin-server")
		t.Setenv("POSTGRES_SSLMODE", "require")
		dsn, err := postgresDSNFromEnv()
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		want := "postgres://my+user:p%40ss%2Fword@db.local:5434/go-gin-server?sslmode=require"
		if dsn != want {
...
// tested without a live database. Values are copied into the scan destinations
// by reflection, so their concrete types must match the columns being scanned.
type fakeRow struct {
	values []any
	err    error
}
