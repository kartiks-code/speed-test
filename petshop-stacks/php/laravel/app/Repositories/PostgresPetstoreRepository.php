...
class PostgresPetstoreRepository implements PetstoreRepositoryInterface
{
    ...
    public function addPet(array $pet): array
    {
        ...
        if ($id === null || $id === 0) {
            $stmt = $this->pdo->query("SELECT nextval('pet_id_seq')");
    ...
    {
        $id_stmt = $this->pdo->query("SELECT nextval('pet_photo_id_seq')");
    ...
    public function placeOrder(array $order): array
    {
        ...
        if ($id === null || $id === 0) {
            $stmt = $this->pdo->query("SELECT nextval('order_id_seq')");
            $id   = (int) $stmt->fetchColumn();
        }
    ...
    public function createUser(array $user): array
    {
        ...
        if ($id === null || $id === 0) {
            $stmt = $this->pdo->query("SELECT nextval('user_id_seq')");
            $id   = (int) $stmt->fetchColumn();
        }
