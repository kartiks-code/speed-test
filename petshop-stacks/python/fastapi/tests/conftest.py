...
* server-assigned ids use Postgres sequences (``nextval``).
...
class FakeDB:
    ...
    def __init__(self) -> None:
        ...
        self._pet_photo_seq: int = 0

    def nextval(self, seq: str) -> int:
        if seq == "pet_id_seq":
            self._pet_seq += 1
            return self._pet_seq
        if seq == "order_id_seq":
            self._order_seq += 1
            return self._order_seq
        if seq == "pet_photo_id_seq":
            self._pet_photo_seq += 1
            return self._pet_photo_seq
        raise NotImplementedError(f"unknown sequence: {seq!r}")
...
class FakeConnection:
    ...
    async def fetchval(self, query: str, *args):
        q = _norm(query)
        if "nextval('pet_id_seq')" in q:
            return self.db.nextval("pet_id_seq")
        if "nextval('order_id_seq')" in q:
            return self.db.nextval("order_id_seq")
